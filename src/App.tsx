    ...
    >
      ...
      <nav className="fixed top-0 left-0 w-full bg-white/80 dark:bg-[#1e293b]/90 shadow z-50 backdrop-blur border-b border-[#f3f4f6] dark:border-[#1e293b]">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 py-2">
          <span className="font-bold text-[#1e40af] dark:text-[#60a5fa] text-lg tracking-wide">CICOM</span>
          ...
          >
            {menuOpen ? (
              <svg className="h-6 w-6 text-[#1e40af] dark:text-[#60a5fa]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
            ) : (
              <svg className="h-6 w-6 text-[#1e40af] dark:text-[#60a5fa]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
            )}
          </button>
          {/* Menú de secciones */}
          <ul className={`
          ...
          `}>
            {sections.map((s) => (
              <li key={s.id} className="border-b md:border-none border-[#f3f4f6] dark:border-[#1e293b] md:mb-0">
                <a
                  href={`#${s.id}`}
          ...
          >
            {dark ? (
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#1e40af]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m8.66-13.66l-.71.71M4.05 19.07l-.71.71M21 12h-1M4 12H3m16.66 6.66l-.71-.71M4.05 4.93l-.71-.71M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
            ) : (
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#1e40af]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12.79A9 9 0 1111.21 3a7 7 0 109.79 9.79z" /></svg>
            )}
          </button>
        </div>
      </nav>
      ...
      <footer className="bg-[#1e40af] dark:bg-[#60a5fa] text-white dark:text-[#1e40af] text-center py-4 mt-8 shadow-inner">
        © {new Date().getFullYear()} CICOM - Colegio de Ingenieros Comerciales, Financieros y de Marketing de Santa Cruz
      </footer>
    </div>
  );
};
